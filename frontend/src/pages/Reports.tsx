...
const Reports = () => {
  ...
  const handleExportCSV = () => {
    ...
    csvContent += `Total Hajri,${report.totalHajri}\n`;
    csvContent += `Total Earned,${report.totalEarned / 100}\n`;
    csvContent += `Advance,${report.advance / 100}\n`;
    csvContent += `Total Paid,${report.totalPaid / 100}\n`;
    csvContent += `Balance,${report.balance / 100}\n`;

    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", `employee_report_${month}.csv`);
